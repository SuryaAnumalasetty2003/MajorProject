...
namespace SeleniumTesting_Project
{
    ...
    public class SearchAndViewEndToEnd
    {
        ...
        public void SearchAllocation_VerifyLotDetails_IsNotPresent()
        {
            ...
            // 4. Wait for either results OR "no allocations" message
            bool noResultsMessageDisplayed = false;
            try
            {
                var noResultsElement = wait.Until(ExpectedConditions.ElementIsVisible(By.XPath("//*[contains(text(),'No allocations found')]")));
                noResultsMessageDisplayed = noResultsElement.Displayed;
            }
            catch (WebDriverTimeoutException)
            {
                // No message, maybe allocations are present
            }

            var allocationCards = driver.FindElements(By.CssSelector("div.bg-white.border-l-4"));

            bool notFoundInCards = allocationCards.All(card =>
                card.Text.IndexOf(searchTerm, StringComparison.OrdinalIgnoreCase) < 0);

            // 5. Assert
            Assert.IsTrue(noResultsMessageDisplayed || notFoundInCards,
                $"Expected no allocations for '{searchTerm}', but some matching results were found.");
        }
